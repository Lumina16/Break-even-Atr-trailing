//+------------------------------------------------------------------+
//|                                    XAUUSD_M5M1_Scalper_V1_03.mq5 |
//|                                     XAUUSD M5/M1 Scalper  V1.03  |
...
#property description "V1.03 adds same-direction re-entry control: after a managed trade closes, the"
#property description "same direction rearms only after the pullback condition clears and a NEW pullback"
#property description "freshly qualifies; ReentryCooldownMinutes stays as a small safety timer only."
...
enum ENUM_ENTRY_BLOCK
  {
   ...
   // V1.03: final safety cooldown after a same-direction trade close.
   // Freshness is decided by the rearm pullback cycle, never by this timer.
   ENTRY_BLOCK_REENTRY_COOLDOWN = 17
  };
...
input double PullbackATRMin         = 0.80;
input int    PullbackReferenceBars  = 3;
input int    ReclaimLookbackBars    = 3;   // Legacy V1.01 compatibility; unused by V1.02 turn-candle trigger
input int    SetupExpiryMinutes     = 13;
input int    ReentryCooldownMinutes = 2;

input group "TRADE MANAGEMENT"
input double InitialStopATRBuffer = 0.50;
input double MinInitialRiskATR    = 1.30;
input double MaxInitialRiskATR    = 1.40;
input double BreakEvenTriggerATR  = 0.80;
input double BreakEvenLockATR     = 0.24;
input double TrailStartATR        = 1.40;
input double TrailATRMult         = 0.40;

input group "VOLATILITY"
input int    VolatilityLookback   = 40;
...
int    EffATRPeriod              = 20;
double EffPullbackATRMin         = 0.80;
int    EffPullbackReferenceBars  = 3;
int    EffReclaimLookbackBars    = 3;
int    EffSetupExpiryMinutes     = 13;
int    EffReentryCooldownMinutes = 2;
double EffInitialStopATRBuffer   = 0.50;
double EffMinInitialRiskATR      = 1.30;
double EffMaxInitialRiskATR      = 1.40;
double EffBreakEvenTriggerATR    = 0.80;
double EffBreakEvenLockATR       = 0.24;
double EffTrailStartATR          = 1.40;
double EffTrailATRMult           = 0.40;
int    EffVolatilityLookback     = 40;
double EffVolatilitySpikeLimit   = 1.50;
double EffLotSize                = 0.01;
int    EffMaxOpenTrades          = 1;
int    EffMaxSpreadPoints        = 120;
int    EffMaxSlippagePoints      = 100;
...
struct SStats
  {
   ...
   // V1.03 same-direction re-entry control (diagnostic only)
   long              buyRearmLocks;
   long              sellRearmLocks;
   long              buyPullbackClearsAfterTrade;
   long              sellPullbackClearsAfterTrade;
   long              buyFreshPullbacksAfterTrade;
   long              sellFreshPullbacksAfterTrade;
   long              reentryCooldownBlocks;
   // entry pipeline
   long              entryAttempts;
...
// V1.03: per-direction same-direction re-entry (rearm) state.
// After a managed trade fully closes, its direction locks; the SAME
// pullback predicate used for normal arming must first be observed FALSE
// (old episode cleared) and then freshly FALSE -> TRUE (new episode)
// before that direction may arm again. Time alone never unlocks.
struct SRearmState
  {
   bool              locked;                   // waiting for clear + fresh qualification
   bool              sawClearedState;          // pullback predicate observed FALSE after the close
   bool              previousPullbackQualified;// predicate value of the previous observation
   datetime          lastTradeCloseTime;       // actual full-close deal time (cooldown anchor)
   long              lastExitPositionId;       // position id of the close that created the lock
  };
SRearmState g_rearmBuy;
SRearmState g_rearmSell;
...
// --- V1.03 same-direction re-entry (rearm) engine ---------------------------
void ZeroRearmState(SRearmState &rs)
  {
   rs.locked                    = false;
   rs.sawClearedState           = false;
   rs.previousPullbackQualified = false;
   rs.lastTradeCloseTime        = 0;
   rs.lastExitPositionId        = 0;
  }

string RearmStateText(const SRearmState &rs)
  {
   if(!rs.locked) return "READY";
   return (rs.sawClearedState ? "WAIT_NEW_PB" : "WAIT_CLEAR");
  }

// Activated exactly once when a managed position on this symbol fully closes.
// The closing DEAL side is irrelevant here: the direction comes from the
// position's opening deal type. If the pullback predicate is still TRUE at
// the close moment, the direction waits for an observed clear; if already
// FALSE, the old episode is considered cleared immediately.
   ...
   if(dir == DIR_BUY)
     {
      ...
      g_rearmBuy.sawClearedState           = !currentQualified;
      g_rearmBuy.previousPullbackQualified = currentQualified;
      g_rearmBuy.lastTradeCloseTime        = closeTime;
      g_rearmBuy.lastExitPositionId        = exitPositionId;
      g_stats.buyRearmLocks++;
     }
   else
     {
      g_rearmSell.locked                    = true;
      g_rearmSell.sawClearedState           = !currentQualified;
      g_rearmSell.previousPullbackQualified = currentQualified;
      g_rearmSell.lastTradeCloseTime        = closeTime;
      g_rearmSell.lastExitPositionId        = exitPositionId;
      g_stats.sellRearmLocks++;
     }
   if(DebugM1Enabled())
      PrintFormat("[REARM][%s] Trade closed at %s -> %s",
                  DirLabel(dir), TimeToString(closeTime, TIME_DATE | TIME_SECONDS),
                  (currentQualified ? "existing pullback still active -> WAIT_CLEAR"
                                    : "pullback already clear -> WAIT_NEW_PULLBACK"));
  }
      ...
      if(!g_rearmBuy.sawClearedState)
        {
         if(!current)
           {
            g_rearmBuy.sawClearedState = true;
            g_stats.buyPullbackClearsAfterTrade++;
            if(DebugM1Enabled())
               Print("[REARM][BUY] Pullback cleared -> WAIT_NEW_PULLBACK");
           }
        }
      else if(!g_rearmBuy.previousPullbackQualified && current)
        {
         g_rearmBuy.locked = false;
         g_stats.buyFreshPullbacksAfterTrade++;
         if(DebugM1Enabled())
            Print("[REARM][BUY] Fresh pullback qualification detected -> UNLOCKED");
        }
      g_rearmBuy.previousPullbackQualified = current;
     }
      ...
      if(!g_rearmSell.sawClearedState)
        {
         if(!current)
           {
            g_rearmSell.sawClearedState = true;
            g_stats.sellPullbackClearsAfterTrade++;
            if(DebugM1Enabled())
               Print("[REARM][SELL] Pullback cleared -> WAIT_NEW_PULLBACK");
           }
        }
      else if(!g_rearmSell.previousPullbackQualified && current)
        {
         g_rearmSell.locked = false;
         g_stats.sellFreshPullbacksAfterTrade++;
         if(DebugM1Enabled())
            Print("[REARM][SELL] Fresh pullback qualification detected -> UNLOCKED");
        }
      g_rearmSell.previousPullbackQualified = current;
     }
  }
   ...
   for(int i = 0; i < total; i++)
     {
      ...
      if(entry != DEAL_ENTRY_OUT && entry != DEAL_ENTRY_OUT_BY) continue; // never entry deals
      ...
      for(int d = 0; d < deals; d++)
        {
         ...
         if(de == DEAL_ENTRY_IN && inType == 0)
            inType = HistoryDealGetInteger(dt, DEAL_TYPE); // original position direction
   ...
   switch(block)
     {
      ...
      case ENTRY_BLOCK_REENTRY_COOLDOWN: return "same-direction reentry cooldown active";
   ...
   switch(block)
     {
      ...
      case ENTRY_BLOCK_REENTRY_COOLDOWN: g_stats.reentryCooldownBlocks++; break;
...
ENUM_ENTRY_BLOCK CheckEntryPermissions(const MqlTick &tick)
  {
   ...
   // 4b. V1.03: final safety cooldown measured from the actual same-direction
   // trade close. Freshness is already guaranteed by the rearm cycle; this
   // timer only suppresses immediate post-trade noise. A cooldown of 0 leaves
   // the fresh-pullback requirement fully in force.
   datetime rearmClose = (g_setup.direction == DIR_BUY) ? g_rearmBuy.lastTradeCloseTime
                                                        : g_rearmSell.lastTradeCloseTime;
   if(rearmClose > 0 &&
      (long)(TimeCurrent() - rearmClose) < (long)EffReentryCooldownMinutes * 60)
      return ENTRY_BLOCK_REENTRY_COOLDOWN;
...
void AttemptEntry(const MqlTick &tick)
  {
   ...
   if(block == ENTRY_OK)
   ...
   else
     {
      ...
      if(block == ENTRY_BLOCK_REENTRY_COOLDOWN)
        {
         // V1.03: fired once per retired trigger episode; no deferred entry
         // will be made when the cooldown expires — a fresh crossing is required.
         datetime rearmClose = (g_setup.direction == DIR_BUY) ? g_rearmBuy.lastTradeCloseTime
                                                              : g_rearmSell.lastTradeCloseTime;
         long remaining = (long)EffReentryCooldownMinutes * 60 - (long)(TimeCurrent() - rearmClose);
         if(remaining < 0) remaining = 0;
         PrintFormat("[REARM][%s] Entry blocked: cooldown remaining %d sec",
                     DirLabel(g_setup.direction), (int)remaining);
        }
      SetLastAction("Entry blocked: " + EntryBlockReason(block));
     }

   // retire the episode no matter what happened (executed, blocked, or failed)
   ResetSetup();
  }
...
void BuildPanel()
  {
   EnsurePanelBackground(PANEL_PREFIX + "BG", 8, 8, 320, 390);
   EnsurePanelLabel(PANEL_PREFIX + "TITLE", 16, 14, "XAUUSD M5/M1 SCALPER V1.03", C'0,210,255', 10);
...
void UpdatePanel(const MqlTick &tick)
  {
   ...
   // --- row 22: V1.03 per-direction rearm state (display only)
   string rearmText = RearmStateText(g_rearmBuy) + " / " + RearmStateText(g_rearmSell);
   color  rearmColor = (g_rearmBuy.locked || g_rearmSell.locked) ? C'255,193,7' : clrWhite;
   PanelRow(22, "Rearm B/S", rearmText, rearmColor);
  }
...
void PrintBacktestSummary()
  {
   ...
   Print("Rearm (V1.03 same-direction re-entry):");
   Print("  Rearm locks B/S:        ", g_stats.buyRearmLocks, " / ", g_stats.sellRearmLocks);
   Print("  Pullback clears B/S:    ", g_stats.buyPullbackClearsAfterTrade, " / ", g_stats.sellPullbackClearsAfterTrade);
   Print("  Fresh pullbacks B/S:    ", g_stats.buyFreshPullbacksAfterTrade, " / ", g_stats.sellFreshPullbacksAfterTrade);
   Print("  Reentry cooldown blocks: ", g_stats.reentryCooldownBlocks);
   Print("--------------------------------------------------");
...
void TryCompleteInitialization()
  {
   ...
   // V1.03: one-time deterministic restoration of post-trade rearm locks
   // from recent deal history (no historical trigger/pullback replay).
   RestoreRearmStateFromHistory();

   g_InitComplete = true;
   PrintFormat("[INIT] XAUUSD M5/M1 Scalper V1.03 warmup complete on %s. Regime=%s (%s)  Swings H %s/%s  L %s/%s  ATR(M1)=%s",
...
int OnInit()
  {
   ...
   g_lastM5Candidate = 0;
   g_setupIdCounter = 0;
   ZeroRearmState(g_rearmBuy);   // V1.03: fresh start = both directions unlocked
   ZeroRearmState(g_rearmSell);
