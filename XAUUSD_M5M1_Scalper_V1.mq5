//+------------------------------------------------------------------+
//|                                       XAUUSD_M5M1_Scalper_V1.mq5 |
//|                                     XAUUSD M5/M1 Scalper  V1.00  |
...
#property description "M1 ATR-normalized pullback arms a setup; fresh M1 reclaim cross executes once."
...
// M1  = reclaim / micro-breakout execution (fresh live-price cross)
...
input int    ReclaimLookbackBars    = 3;   // Completed M1 bars defining reclaim level
...
struct SSetup
  {
   bool              active;             // armed, waiting for reclaim cross
   ENUM_SETUP_STATE  state;
   ENUM_TRADE_DIR    direction;
   datetime          setupStartTime;     // = pullback qualification time
   ...
   double            reclaimLevel;       // micro-breakout trigger
   double            atrAtSetup;         // completed M1 ATR at qualification
   bool              crossArmed;         // price last seen on the non-breakout side
   long              setupId;
  };
...
double GetBuyReclaimLevel()
  {
   int idx = iHighest(_Symbol, PERIOD_M1, MODE_HIGH, EffReclaimLookbackBars, 1);
   if(idx < 1) return 0.0;
   double v = iHigh(_Symbol, PERIOD_M1, idx);
   return (v > 0.0 ? v : 0.0);
  }

double GetSellReclaimLevel()
  {
   int idx = iLowest(_Symbol, PERIOD_M1, MODE_LOW, EffReclaimLookbackBars, 1);
   if(idx < 1) return 0.0;
   double v = iLow(_Symbol, PERIOD_M1, idx);
   return (v > 0.0 ? v : 0.0);
  }
...
// Reclaim level updates ONLY on a new completed M1 candle (never per tick).
// Fresh-cross state is re-derived from the live price: if price already sits
// beyond the new level the cross is consumed until price returns back.
void RefreshReclaimLevel()
  {
   MqlTick tick;
   if(!SymbolInfoTick(_Symbol, tick)) return;
   double level = (g_setup.direction == DIR_BUY) ? GetBuyReclaimLevel() : GetSellReclaimLevel();
   if(level <= 0.0) return;
   bool changed = (MathAbs(level - g_setup.reclaimLevel) > TickSizeSafe() * 0.5);
   g_setup.reclaimLevel = level;
   if(g_setup.direction == DIR_BUY)
      g_setup.crossArmed = (tick.ask <= level);
   else
      g_setup.crossArmed = (tick.bid >= level);
   if(changed)
      LogM1(StringFormat("%s reclaim level updated -> %s  (crossArmed=%s)",
                         DirLabel(g_setup.direction), DoubleToString(level, _Digits),
...
bool ArmSetup(const ENUM_TRADE_DIR dir, const MqlTick &tick)
  {
   ...
   double reclaim = (dir == DIR_BUY) ? GetBuyReclaimLevel() : GetSellReclaimLevel();
   if(reclaim <= 0.0) return false;
   ...
   g_setup.reclaimLevel          = reclaim;
   g_setup.atrAtSetup            = g_atrPrev;
   g_setup.crossArmed            = (dir == DIR_BUY) ? (tick.ask <= reclaim) : (tick.bid >= reclaim);
   g_setupIdCounter++;
   ...
   if(dir == DIR_BUY)
     {
      ...
      PrintFormat("[M1] BUY pullback qualified (#%I64d)  ReferenceHigh=%s  CurrentBid=%s  Distance=%s  ATR=%s  Required=%s  Reclaim=%s",
                  ...
                  DoubleToString(reclaim, _Digits));
     }
   else
     {
      double dist = tick.ask - refExtreme;
      double req  = g_atrPrev * EffPullbackATRMin;
      PrintFormat("[M1] SELL pullback qualified (#%I64d)  ReferenceLow=%s  CurrentAsk=%s  Distance=%s  ATR=%s  Required=%s  Reclaim=%s",
...
// WAIT_RECLAIM processing: expiry, monotonic pullback extreme, fresh cross.
void ProcessLiveSetup(const MqlTick &tick)
  {
   ...
   // --- fresh crossing detection (live prices; BUY=Ask, SELL=Bid)
   bool crossed = false;
   if(g_setup.direction == DIR_BUY)
     {
      if(tick.ask <= g_setup.reclaimLevel)
         g_setup.crossArmed = true;                 // price on the non-breakout side
      else if(g_setup.crossArmed)
         crossed = true;                            // genuine up-cross
     }
   else
     {
      if(tick.bid >= g_setup.reclaimLevel)
         g_setup.crossArmed = true;
      else if(g_setup.crossArmed)
         crossed = true;                            // genuine down-cross
     }

   if(crossed)
     {
      g_setup.crossArmed = false;      // episode consumed immediately
      g_stats.reclaimCrossings++;
      AttemptEntry(tick);
     }
  }
...
// Full 14-step permission pipeline at the instant of a fresh reclaim cross.
ENUM_ENTRY_BLOCK CheckEntryPermissions(const MqlTick &tick)
  {
   ...
   if(marginReq > AccountInfoDouble(ACCOUNT_MARGIN_FREE))
      return ENTRY_BLOCK_MARGIN;
   // 13. legally placeable protective stop (atomic, included in the order)
   // ATR_at_entry = latest completed M1 ATR immediately before entry
   g_stopBlockReason = "";
   double sl = 0.0;
   if(!BuildInitialStop(g_setup.direction, tick, GetATR(), g_setup.pullbackExtreme, sl, g_stopBlockReason))
...
// One reclaim crossing = one entry episode. Executed, blocked, or failed:
...
void AttemptEntry(const MqlTick &tick)
  {
   ...
   if(g_setup.direction == DIR_BUY)
      PrintFormat("[M1] BUY reclaim fresh-cross detected  PrevAsk=%s  Ask=%s  Trigger=%s",
                  DoubleToString(g_prevAsk, _Digits), DoubleToString(tick.ask, _Digits),
                  DoubleToString(g_setup.reclaimLevel, _Digits));
   else
      PrintFormat("[M1] SELL reclaim fresh-cross detected  PrevBid=%s  Bid=%s  Trigger=%s",
...
void BuildPanel()
  {
   EnsurePanelBackground(PANEL_PREFIX + "BG", 8, 8, 320, 344);
   EnsurePanelLabel(PANEL_PREFIX + "TITLE", 16, 14, "XAUUSD M5/M1 SCALPER V1.00", C'0,210,255', 10);
...
void UpdatePanel(const MqlTick &tick)
  {
   ...
   if(!g_InitComplete)
     ...
     {
      stateText = "WAIT_RECLAIM " + DirLabel(g_setup.direction);
   ...
   // --- row 10: reclaim level
   PanelRow(10, "Reclaim Level",
            (g_setup.active && g_setup.reclaimLevel > 0.0
             ? DoubleToString(g_setup.reclaimLevel, _Digits) : "-"), clrWhite);
...
void TryCompleteInitialization()
  {
   ...
   int m1Need = MathMax(EffPullbackReferenceBars,
                        MathMax(EffReclaimLookbackBars, EffVolatilityLookback + 2)) + 2;
   ...
   g_InitComplete = true;
   PrintFormat("[INIT] XAUUSD M5/M1 Scalper V1.00 warmup complete on %s. Regime=%s (%s)  Swings H %s/%s  L %s/%s  ATR(M1)=%s",
...
void ProcessNewM1Bar()
  {
   ...
   if(g_setup.active && g_setup.state == WAIT_RECLAIM)
      RefreshReclaimLevel();     // reclaim level updates only here (never per tick)
  }
...
void OnTick()
  {
   ...
   // 3. new M1 bar: refresh ATR / volatility / reference / reclaim caches
   if(IsNewBar(PERIOD_M1, g_lastM1BarTime))
      ProcessNewM1Bar();

   // 4. live M1 pullback / reclaim engine
