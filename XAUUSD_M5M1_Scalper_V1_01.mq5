//+------------------------------------------------------------------+
//|                                    XAUUSD_M5M1_Scalper_V1_01.mq5 |
//|                                     XAUUSD M5/M1 Scalper  V1.01  |
   ...
   if(block == ENTRY_OK)
   ...
   else
     {
      ...
      if(block == ENTRY_BLOCK_RISK_TOO_TIGHT || block == ENTRY_BLOCK_RISK_TOO_WIDE)
        {
         // V1.01 risk-geometry detail (fired once per retired reclaim episode only)
...
void BuildPanel()
  {
   EnsurePanelBackground(PANEL_PREFIX + "BG", 8, 8, 320, 360);
   EnsurePanelLabel(PANEL_PREFIX + "TITLE", 16, 14, "XAUUSD M5/M1 SCALPER V1.01", C'0,210,255', 10);
...
void TryCompleteInitialization()
  {
   ...
   g_InitComplete = true;
   PrintFormat("[INIT] XAUUSD M5/M1 Scalper V1.01 warmup complete on %s. Regime=%s (%s)  Swings H %s/%s  L %s/%s  ATR(M1)=%s",
